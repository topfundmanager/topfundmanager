import { buildPublishedLlms, getPublishedProfile, isSiteId, TEXT_HEADERS } from '../../published-utils.js';
...
export async function onRequestGet({ env, params }) {
  ...
  try {
    ...
    return new Response(buildPublishedLlms(profile), {
      status: 200,
      headers: { ...TEXT_HEADERS, 'X-SEO-Revision': String(profile.revision) },
    });
  } catch (error) {
    console.error('Published llms.txt read failed:', error);
